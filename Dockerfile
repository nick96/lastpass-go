...
CMD go test -coverprofile=coverage.txt -covermode count -timeout 1s -v ./... 2>&1 | tee gotest.out && \
	go-junit-report <gotest.out >/output/gotest.xml && \
	gocover-cobertura <coverage.txt >/output/coverage.xml
